...
Поясните ответ.  
**Ответ.** Нарушения нет. Так как ``RuntimeException`` является ``Exception``.  
...
**Ответ.** Статические методы в интерфейсе являются частью интерфейса, мы не можем использовать его для объектов класса реализации.  
**Источник.** https://javadevblog.com/interfejsy-v-java-8-staticheskie-metody-metody-po-umolchaniyu-funktsional-ny-e-interfejsy.html  
