...
```  
Сегмент помеченый как `code` в примере содержит одну или несколько допустимых строк кода, которые могут вызвать исключение.  
...
**Ответ.** Обработчики исключений связываются  с блоком ``try``, предоставляя один или несколько блоков ``catch`` непосредственно после блока ``try``. Между концом блока `try`` и началом первого блока ``catch`` не может быть никакого кода.  
**Источник.** https://docs.oracle.com/javase/tutorial/essential/exceptions/catch.html  
...
**Ответ.** Оператор ``try-with-resources`` может иметь блоки ``catch`` и ``finally``, как и обычный оператор ``try``. В операторе ``try-with-resources`` любой блок ``catch`` или ``finally`` запускается после того, как объявленные ресурсы были закрыты.  
**Источник.** https://question-it.com/tags/try-with-resources/info  
...
**Ответ.** Cледует создать собственное исключение как подкласс класса Exception либо RuntimeException должен заканчиваться словом Exception.  
**Источник.** Java from EPAM : учеб.-метод. пособие / И. Н. Блинов, В. С. Романчик, 2020. – 274 с.  
